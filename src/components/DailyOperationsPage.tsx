...
// NOTE: This combined component (mode="opening" | "closing") overlaps in
// responsibility with the dedicated DailyOpeningPage / DailyClosingPage
// components. If your routing only points at those two, this file can be
// retired; it's kept fully functional here in case it's still linked from
// anywhere in the app.
...
// language with Transactions/Transfers/Wallets pages.
// ============================================================================
...
function CurrencyBadge({ code, size = 20 }: { code: string; size?: number }) {
  const clipId = `daily-ops-flag-${code}`;
  return (
...
// to each option — native <option> elements only support plain text.
...
// Real-time forex rates poll on this interval while the page is open, on top
// of the manual refresh button — so opening/closing math always reflects a
// fresh rate without staff having to remember to hit refresh.
...
function ChannelIcon({ channelKey, className }: { channelKey: string; className?: string }) {
  if (channelKey.startsWith('cash_') || channelKey === 'cash') return <Banknote className={className} />;
...
// Gives each channel type a distinct brand-color accent instead of a flat
// white card for every row — cash = teal, mobile money = orange, bank =
// purple, everything else = neutral slate.
function channelAccent(key: string): { border: string; iconBg: string; iconColor: string } {
  if (key.startsWith('cash_') || key === 'cash') {
    return { border: 'border-t-[#1ebcb2]', iconBg: 'bg-[#1ebcb2]/10', iconColor: 'text-[#1ebcb2]' };
  }
  if (key === 'mtn_momo' || key === 'mpesa') {
    return { border: 'border-t-[#ee7b22]', iconBg: 'bg-[#ee7b22]/10', iconColor: 'text-[#ee7b22]' };
  }
  if (key === 'bank') {
    return { border: 'border-t-[#641f60]', iconBg: 'bg-[#641f60]/10', iconColor: 'text-[#641f60]' };
  }
  return { border: 'border-t-slate-300', iconBg: 'bg-slate-100', iconColor: 'text-slate-500' };
}

type PillTone = 'warn' | 'success' | 'info' | 'danger';

function StatusPill({ label, tone }: { label: string; tone: PillTone }) {
  const styles: Record<PillTone, string> = {
    warn: 'bg-[#ee7b22]/10 text-[#ee7b22] border-[#ee7b22]/30',
    success: 'bg-[#1ebcb2]/10 text-[#1ebcb2] border-[#1ebcb2]/30',
    info: 'bg-[#641f60]/10 text-[#641f60] border-[#641f60]/30',
    danger: 'bg-[#c46040]/10 text-[#c46040] border-[#c46040]/30',
  };
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${styles[tone]}`}>
...
export function DailyOperationsPage({ mode }: DailyOperationsProps) {
  ...
  // Opening-only: which currencies this branch is handling today, and how
  // each reference account is configured — replaces the old hardcoded
  // usd/kes/ssp float types with real, tenant-configurable currencies.
  ...
  const tenantSettings = useMemo<TenantSettingsShape>(() => (tenant?.settings ?? {}) as TenantSettingsShape, [tenant]);
  ...
  // Closing mode always uses whatever currency configuration was locked in
  // at opening (persisted on tenant settings), never a hardcoded guess.
  const closingBaseCurrency = tenantDefaultBase;
  const closingAccountCurrencies = tenantDefaultAccountCurrencies;

  useEffect(() => {
    if (!settingsInitialized.current && tenant && isOpening) {
  ...
  }, [tenant, isOpening, tenantDefaultCurrencies, tenantDefaultBase, tenantDefaultAccountCurrencies]);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const operation = todayRow as DailyOperation | null;
      setTodaysOp(operation);
      ...
      if (isOpening) {
        ...
        const rows5 = (priorRows ?? []) as DailyOperation[];
        setUnclosedPriorOp(rows5.find((r) => r.state !== 'closed') ?? null);
        const lastClosed = rows5.find((r) => r.state === 'closed');
        setLastClosingBalances(lastClosed ? parseBalances(lastClosed.closing_balances) : {});
      } else if (operation && operation.state === 'active') {
        const opening = parseBalances(operation.opening_balances);
        setRows(
      ...
      setLoadError(err instanceof Error ? err.message : 'Failed to load daily operations data');
    } finally {
      setLoading(false);
    }
  }, [tenant, branch, isOpening]);
  ...
  // Rebuild opening rows whenever the selected currencies change (opening
  // mode only, and only while nothing has been submitted yet today).
  useEffect(() => {
    if (!isOpening || todaysOp) return;
    setRows((prev) => {
      const existingMap = new Map(prev.map((r) => [r.key, r.amount]));
      const valueFor = (key: string) => existingMap.get(key) ?? (lastClosingBalances[key] !== undefined ? String(lastClosingBalances[key]) : '');
      const currencyRows: BalanceRow[] = selectedCurrencies.map((c) => ({
        key: `cash_${c}`,
        label: `Cash (${c})`,
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isOpening, selectedCurrencies, lastClosingBalances, todaysOp]);
  ...
  const handleSaveDefaultCurrencies = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('tenants').update({ settings: updatedSettings }).eq('id', tenant.id);
  ...
  const activeBaseCurrency = isOpening ? baseCurrency : closingBaseCurrency;
  const activeAccountCurrencies = isOpening ? accountCurrencies : closingAccountCurrencies;

  const resolvedRate = useCallback(
    (code: string): number | null => {
      if (code === activeBaseCurrency) return 1;
      const fx = lookupRate(fxRates, code, activeBaseCurrency, 'buy');
      if (fx !== null) return fx;
      const manual = parseFloat(manualRates[code] ?? '');
      return Number.isFinite(manual) && manual > 0 ? manual : null;
    },
    [fxRates, activeBaseCurrency, manualRates]
  );

  const rateSource = useCallback(
    (code: string): 'base' | 'forex' | 'manual' | 'missing' => {
      if (code === activeBaseCurrency) return 'base';
      if (lookupRate(fxRates, code, activeBaseCurrency, 'buy') !== null) return 'forex';
      const manual = parseFloat(manualRates[code] ?? '');
  ...
  const activeCurrencies = useMemo(() => {
    ...
    if (isOpening) {
      selectedCurrencies.forEach((c) => set.add(c));
      for (const acc of REFERENCE_ACCOUNTS) if (accountCurrencies[acc]) set.add(accountCurrencies[acc]);
    } else {
      rows.forEach((r) => set.add(currencyForKey(r.key, activeAccountCurrencies, activeBaseCurrency)));
    }
    return Array.from(set).sort();
  }, [isOpening, selectedCurrencies, accountCurrencies, rows, activeAccountCurrencies, activeBaseCurrency]);

  const currenciesNeedingRates = activeCurrencies.filter((c) => c !== activeBaseCurrency);
  const missingRateCurrencies = currenciesNeedingRates.filter((c) => rateSource(c) === 'missing');

  const rowsTotalInBase = useMemo(
    () =>
      rows.reduce((sum, r) => {
        const amt = parseFloat(r.amount) || 0;
        const code = currencyForKey(r.key, activeAccountCurrencies, activeBaseCurrency);
        const rate = resolvedRate(code);
        return sum + amt * (rate ?? 0);
      }, 0),
    [rows, activeAccountCurrencies, activeBaseCurrency, resolvedRate]
  );

  const openingTotalInBase = useMemo(() => {
    if (isOpening || !todaysOp) return 0;
    const opening = parseBalances(todaysOp.opening_balances);
    return Object.entries(opening).reduce((sum, [key, value]) => {
      const code = currencyForKey(key, activeAccountCurrencies, activeBaseCurrency);
      const rate = resolvedRate(code);
      return sum + value * (rate ?? 0);
    }, 0);
  }, [isOpening, todaysOp, activeAccountCurrencies, activeBaseCurrency, resolvedRate]);

  const expectedClosingTotal = useMemo(
    () => openingTotalInBase + txSummary.totalCredits - txSummary.totalDebits,
    [openingTotalInBase, txSummary]
  );
  const closingVariance = useMemo(() => rowsTotalInBase - expectedClosingTotal, [rowsTotalInBase, expectedClosingTotal]);
  const isBalanced = Math.abs(closingVariance) < 0.01;
  ...
  const validate = (): string | null => {
    if (isOpening) {
      ...
      if (selectedCurrencies.length === 0) return 'Select at least one currency for today.';
    }
    if (missingRateCurrencies.length > 0) {
      return `No exchange rate found for ${missingRateCurrencies.join(', ')} \u2192 ${activeBaseCurrency}. Add the pair in Forex Trading, or enter a manual rate below.`;
    }
    const hasAnyAmount = rows.some((r) => r.amount.trim() !== '');
    if (!hasAnyAmount) return `Enter at least one ${isOpening ? 'opening' : 'closing'} balance.`;
    for (const r of rows) {
      if (r.amount.trim() !== '' && parseFloat(r.amount) < 0) return `${r.label} cannot be negative.`;
    }
    return null;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isOpening) {
        ...
        // Persist the currency configuration used today so Daily Closing (and
        // any other view) values the same balances with the same currencies.
        try {
          const existingSettings = (tenant.settings ?? {}) as TenantSettings;
          const needsUpdate =
            accountCurrenciesDiffer(accountCurrencies, tenantDefaultAccountCurrencies) ||
          ...
          if (needsUpdate) {
            await supabase
              .from('tenants')
              .update({
                settings: { ...existingSettings, base_currency: baseCurrency, account_currencies: accountCurrencies },
              })
              .eq('id', tenant.id);
          }
        } catch (persistErr) {
        ...
        if (!todaysOp) throw new Error('No daily operation found to close.');
        const { error: updateError } = await supabase
          ...
          .update({
            ...
            approved_at: new Date().toISOString(),
            notes: !isBalanced
              ? `Variance of ${formatMoney(closingVariance, activeBaseCurrency)} recorded at closing (closing total vs expected, converted to ${activeBaseCurrency}).`
              : null,
          })
          .eq('id', todaysOp.id)
          .eq('tenant_id', tenant.id);
      ...
      setFormError(err instanceof Error ? err.message : 'Failed to process operation');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const headerGradient = isOpening ? 'from-[#ee7b22] to-[#c46040]' : 'from-[#641f60] to-[#4a1646]';
  const HeaderIcon = isOpening ? Sunrise : Sunset;

  const statusPill: { label: string; tone: PillTone } = isOpening
    ? unclosedPriorOp
      ? { label: 'Blocked', tone: 'danger' }
      : todaysOp?.state === 'active'
      ? { label: 'Operations Active', tone: 'success' }
      : todaysOp?.state === 'closed'
      ? { label: 'Day Closed', tone: 'info' }
      : { label: 'Pending Opening', tone: 'warn' }
    : !todaysOp
    ? { label: 'Not Opened', tone: 'warn' }
    : todaysOp.state === 'closed'
    ? { label: 'Closed', tone: 'info' }
    : { label: 'Open — Ready to Close', tone: 'success' };
    ...
    <div className="space-y-6 max-w-5xl min-w-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div className="flex items-start gap-4 min-w-0">
          <div className={`p-3.5 rounded-2xl bg-gradient-to-br ${headerGradient} text-white shadow-lg shrink-0`}>
            <HeaderIcon className="w-7 h-7" />
          </div>
          <div className="min-w-0">
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-2xl font-bold text-[#641f60]">Daily {isOpening ? 'Opening' : 'Closing'}</h1>
      ...
      {/* Opening: blocked by an unclosed prior day */}
      {isOpening && unclosedPriorOp && (
        <div className="bg-[#c46040]/5 border border-[#c46040]/30 rounded-xl p-6 flex items-start gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-slate-700 break-words">
              ...
              is still <span className="font-medium capitalize">{unclosedPriorOp.state.replace(/_/g, ' ')}</span>. Close that day
      ...
      {/* Opening: already opened today — read-only summary */}
      {isOpening && !unclosedPriorOp && todaysOp && (
      ...
      {/* Closing: already closed — read-only summary */}
      {!isOpening && todaysOp?.state === 'closed' && (
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          <div className="px-6 py-4 bg-[#641f60]/10 border-b border-[#641f60]/20 flex items-center gap-3">
            <CheckCircle className="w-6 h-6 text-[#641f60] shrink-0" />
            <div className="min-w-0">
              ...
              {Object.entries(parseBalances(todaysOp.closing_balances)).map(([key, value]) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className="font-semibold text-slate-900 text-sm break-words">
                      {formatMoney(value, currencyForKey(key, closingAccountCurrencies, closingBaseCurrency))}
                    </p>
                  </div>
                );
              })}
      ...
      {/* --- Active form: opening (no prior block, not yet opened) --- */}
      {isOpening && !unclosedPriorOp && !todaysOp && (
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200 flex items-start gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm text-slate-500">Choose which currencies this branch will handle today.</p>
              </div>
            </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="pt-2 border-t border-slate-100">
                <p className="text-xs font-medium uppercase tracking-wide text-slate-500 mb-1">Reference account currencies</p>
                  ...
                  {REFERENCE_ACCOUNTS.map((acc) => (
                    <div key={acc} className="border border-slate-200 rounded-lg p-3 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm font-medium text-slate-700 truncate">{REFERENCE_ACCOUNT_LABELS[acc]}</span>
                      </div>
                      ...
                      {source === 'forex' && fx !== null ? (
                        ...
                        <div className="space-y-1">
                          ...
                          <p className="text-xs text-slate-400 flex items-start gap-1">
                            ...
                            Not configured in Forex Trading.
                          </p>
                        </div>
                      )}
                    </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200 flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm text-slate-500">Enter the confirmed cash and float balances to start today&rsquo;s operations.</p>
              </div>
            </div>
            ...
            <div className="p-6 space-y-3">
                ...
                {rows.map((row) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <label htmlFor={`open-${row.key}`} className="block text-center text-xs font-medium text-slate-600 mb-2 truncate" title={row.label}>
                        {row.label}
                      </label>
              ...
              {formError && (
                <div className="p-3 bg-[#c46040]/10 border border-[#c46040]/30 rounded-lg text-[#c46040] text-sm flex items-start gap-2" role="alert">
            ...
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-200 flex flex-wrap items-center justify-between gap-4">
              <div className="min-w-0">
                <p className="text-xs text-slate-500 uppercase tracking-wide">Total, converted to {baseCurrency}</p>
                <p className="text-lg font-semibold text-slate-900 break-words">{formatMoney(rowsTotalInBase, baseCurrency)}</p>
              </div>
              <button
                type="submit"
                disabled={submitting}
      ...
      {/* --- Active form: closing (opened today, not yet closed) --- */}
      {!isOpening && todaysOp?.state === 'active' && (
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid sm:grid-cols-3 gap-4">
            ...
            <div className="bg-white rounded-xl border border-slate-200 border-t-4 border-t-[#1ebcb2] p-4 min-w-0">
              ...
              <p className="text-xl font-bold text-slate-900 break-words">{formatMoney(txSummary.totalCredits, closingBaseCurrency)}</p>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 border-t-4 border-t-[#ee7b22] p-4 min-w-0">
              ...
              <p className="text-xl font-bold text-slate-900 break-words">{formatMoney(txSummary.totalDebits, closingBaseCurrency)}</p>
            </div>
          </div>
          ...
          {currenciesNeedingRates.length > 0 && (
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
              ...
              <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {currenciesNeedingRates.map((code) => {
                  const source = rateSource(code);
                  const fx = lookupRate(fxRates, code, closingBaseCurrency, 'buy');
                  return (
                    <div key={code} className="border border-slate-200 rounded-lg p-3 min-w-0 transition-all hover:shadow-sm">
                      <div className="flex items-center justify-between gap-2 mb-1.5">
                        <span className="text-sm font-medium text-slate-700 whitespace-nowrap flex items-center gap-1.5">
                          <CurrencyBadge code={code} size={16} />1 {code} =
                          <CurrencyBadge code={closingBaseCurrency} size={16} />
                          {closingBaseCurrency}
                        </span>
                        {source === 'forex' && (
                          <span className="text-[10px] font-semibold uppercase tracking-wide px-1.5 py-0.5 rounded bg-[#1ebcb2]/10 text-[#1ebcb2] shrink-0">
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200 flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm text-slate-500">Count and enter actual balances for each channel to close the day.</p>
              </div>
            </div>
            ...
            <div className="p-6 space-y-3">
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                {rows.map((row) => {
                  const rowCurrency = currencyForKey(row.key, closingAccountCurrencies, closingBaseCurrency);
                  const accent = channelAccent(row.key);
                  return (
                    <div
                      key={row.key}
                    ...
                    >
                      ...
                      <label htmlFor={`close-${row.key}`} className="block text-center text-xs font-medium text-slate-600 mb-2 truncate" title={row.label}>
                        {row.label}
                      </label>
                      <div className="relative">
                        <span className="pointer-events-none absolute left-2 top-1/2 -translate-y-1/2 text-xs font-medium text-slate-400">
              ...
              <div className="pt-4 mt-4 border-t border-dashed border-slate-300 space-y-1.5 text-sm">
                <p className="text-xs text-slate-400 mb-1">All figures below converted to {closingBaseCurrency}</p>
                <div className="flex items-center justify-between gap-4 text-slate-600">
                  <span>Opening total</span>
                  <span className="break-words text-right">{formatMoney(openingTotalInBase, closingBaseCurrency)}</span>
                </div>
                <div className="flex items-center justify-between gap-4 text-slate-600">
                  <span>+ Credits &minus; Debits</span>
                  <span className="break-words text-right">
                    {formatMoney(txSummary.totalCredits - txSummary.totalDebits, closingBaseCurrency)}
                  </span>
                </div>
                <div className="flex items-center justify-between gap-4 font-medium text-slate-800">
                  <span>Expected closing total</span>
                  <span className="break-words text-right">{formatMoney(expectedClosingTotal, closingBaseCurrency)}</span>
                </div>
                <div className="flex items-center justify-between gap-4 font-semibold text-slate-900">
                  <span>Entered closing total</span>
                  <span className="break-words text-right">{formatMoney(rowsTotalInBase, closingBaseCurrency)}</span>
                </div>

                <div className={`mt-3 rounded-lg p-3 flex items-start gap-2.5 ${isBalanced ? 'bg-[#1ebcb2]/10' : 'bg-[#c46040]/10'}`}>
                  {isBalanced ? (
                  ...
                  <div className="min-w-0">
                    <p className={`font-semibold ${isBalanced ? 'text-[#1ebcb2]' : 'text-[#c46040]'}`}>
                      {isBalanced
                        ? 'Balances match'
                        : `Off by ${formatMoney(Math.abs(closingVariance), closingBaseCurrency)} (${closingVariance > 0 ? 'over' : 'short'})`}
                    </p>
                    <p className="text-xs text-slate-500 mt-0.5">
                      {isBalanced ? 'Entered totals match expected totals exactly.' : 'This will be recorded as a variance note when you close.'}
                    </p>
                  </div>
                </div>
              </div>
